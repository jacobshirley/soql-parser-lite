...
if [ -n "$1" ]; then
  ...
  # Make sure all packages soql-parser-lite* depend on latest versions of other soql-parser-lite* packages

  echo "Updating all soql-parser-lite* dependencies to version $NEW_VERSION"

  find . -name 'package.json' -not -path '*/node_modules/*' | while read -r pkg_file; do
    # Only update if there are soql-parser-lite* dependencies
    if grep -q '"soql-parser-lite' "$pkg_file"; then
      tmp_file=$(mktemp)
      node -e "
        const fs = require('fs');
        ...
        ['dependencies', 'devDependencies'].forEach(depType => {
          if (pkg[depType]) {
            Object.keys(pkg[depType]).forEach(dep => {
              if (dep.startsWith('soql-parser-lite')) {
