...
if [ -n "$1" ]; then
  ...
  # Make sure all packages soql-parser* depend on latest versions of other soql-parser* packages

  echo "Updating all soql-parser* dependencies to version $NEW_VERSION"

  find . -name 'package.json' -not -path '*/node_modules/*' | while read -r pkg_file; do
    # Only update if there are soql-parser* dependencies
    if grep -q '"soql-parser' "$pkg_file"; then
      tmp_file=$(mktemp)
      node -e "
        const fs = require('fs');
